  ...
  for (let i = 0; i < 6; i++) {
    ...
    for (const state of selectedStates) {
      ...
      if (STATES_WITH_LOCALITIES.includes(state)) {
        ...
        if (numLocalities > 0) {
          stateData.localities = faker.helpers.arrayElements(
            LOCALITY_CODES,
            numLocalities
          );
        }
...
function generateW2(employee, company, stateData, calendarYear) {
  const baseWages = faker.number.float({
    min: 30000,
    max: 200000,
    fractionDigits: 2,
  });
  const stateWages =
    baseWages * faker.number.float({ min: 0.8, max: 1.2, fractionDigits: 2 });
  const localWages1 =
    stateWages * faker.number.float({ min: 0.7, max: 1.0, fractionDigits: 2 });
  const localWages2 =
    stateWages * faker.number.float({ min: 0.1, max: 0.3, fractionDigits: 2 });

  const ssWages = Math.min(baseWages, 160200); // Social Security wage base limit
  const medicareWages = baseWages;

  const federalTax =
    baseWages * faker.number.float({ min: 0.12, max: 0.22, fractionDigits: 2 });
  const stateTax =
    stateWages *
    faker.number.float({ min: 0.02, max: 0.06, fractionDigits: 2 });
  const ssTax = ssWages * 0.062;
  const medicareTax = medicareWages * 0.0145;
  const localTax1 =
    localWages1 *
    faker.number.float({ min: 0.008, max: 0.015, fractionDigits: 2 });
  const localTax2 =
    localWages2 *
    faker.number.float({ min: 0.008, max: 0.015, fractionDigits: 2 });
  ...
  const box12Codes = faker.helpers.arrayElements(
    BOX_12_CODES,
  ...
  if (box12Codes.length > 0) {
    box12Values.a = roundCurrencyToString(
      faker.number.float({ min: 1000, max: 10000, fractionDigits: 2 })
    );
  }
  if (box12Codes.length > 1) {
    box12Values.b = roundCurrencyToString(
      faker.number.float({ min: 1000, max: 10000, fractionDigits: 2 })
    );
  }
  if (box12Codes.length > 2) {
    box12Values.c = roundCurrencyToString(
      faker.number.float({ min: 1000, max: 30000, fractionDigits: 2 })
    );
  }
  ...
  let stateIDNumber = `W-2 for ${employee.firstName} ${employee.lastName} (${
    employee.id
  }) for ${company.name} for the ${calendarYear} ${getStateName(
    stateData.code
  )}`;
  ...
  const w2 = {
    ...
    Box_14_group: roundCurrencyToString(
    ...
    "XMLNAME_10_-_Dependent_care_benefits_group": roundCurrencyToString(
      faker.number.float({ min: 0, max: 5000, fractionDigits: 2 })
    ),
    "XMLNAME_13_-__Retirement_plan_Box_group": intToString(
      faker.number.int({ min: 0, max: 1 })
    ),
    "XMLNAME_15_-_Payroll_Tax_State_Code": stateData.code,
    "XMLNAME_15_-_State_-_Employer_s_State_ID_Number": stateIDNumber,
    "XMLNAME_15_-_State_-_Employer_s_state_ID_number__Previous_":
      stateData.stateID,
    "XMLNAME_16_-_State_Wages_tips__etc_group":
      roundCurrencyToString(stateWages),
    "XMLNAME_17_-_State_income_tax_group": roundCurrencyToString(stateTax),
    "XMLNAME_1_-_Wages__tips__and_other_compensation_group":
      roundCurrencyToString(baseWages),
    "XMLNAME_2_-_Federal_income_tax_withheld_group":
      roundCurrencyToString(federalTax),
    "XMLNAME_3_-_Social_security_wages_group": roundCurrencyToString(ssWages),
    "XMLNAME_4_-_Social_security_tax_withheld_group":
      roundCurrencyToString(ssTax),
    "XMLNAME_5_-_Medicare_wages_and_tips_group":
      roundCurrencyToString(medicareWages),
    "XMLNAME_6_-_Medicare_tax_withheld_group":
      roundCurrencyToString(medicareTax),
  ...
  if (box12Codes.length > 3) {
    w2[`XMLNAME_12d`] = roundCurrencyToString(
      faker.number.float({ min: 0, max: 5000, fractionDigits: 2 })
    );
  } else {
    w2[`XMLNAME_12d`] = "0";
  ...
  if (locality1) {
    w2["XMLNAME_18_-_Local_wages__tips__etc_1"] =
      roundCurrencyToString(localWages1);
    w2["XMLNAME_19_-_Local_income_tax_1"] = roundCurrencyToString(localTax1);
    w2["XMLNAME_20_-_Locality_name_1"] = locality1;
  }
  if (locality2) {
    w2["XMLNAME_18_-_Local_wages__tips__etc_2"] =
      roundCurrencyToString(localWages2);
...
function generateW2C(w2, calendarYear) {
  ...
  const originalStateWages = parseFloat(
    w2["XMLNAME_16_-_State_Wages_tips__etc_group"]
  );
  ...
  const stateIDNumber =
    w2["XMLNAME_15_-_State_-_Employer_s_state_ID_number__Previous_"] ||
  ...
  const w2c = {
    ...
    Calendar_Year: `${calendarYear}-01-01`,
    Company: w2.Company,
    Created_Moment: `${parseInt(calendarYear) + 1}-04-10T${String(
      faker.number.int({ min: 10, max: 15 })
    ).padStart(2, "0")}:${faker.string.numeric(2)}:${faker.string.numeric(
      2
    )}.${faker.string.numeric(3)}-07:00`,
    ...
    "XMLNAME_15_-_Payroll_Tax_State_Code":
      w2["XMLNAME_15_-_Payroll_Tax_State_Code"],
    "XMLNAME_15_-_State_-_Employer_s_State_ID_Number": stateIDNumber,
    "XMLNAME_15_-_State_-_Employer_s_State_ID_Number_W2C": shouldChangeStateID
      ? generateStateID()
      : stateIDNumber,
    "XMLNAME_16_-_State_Wages_tips__etc_group":
      roundCurrencyToString(originalStateWages),
    "XMLNAME_16_-_State_Wages_tips__etc_group_W2C":
      roundCurrencyToString(correctedStateWages),
    "b_-_Employer_identification_number__EIN_":
      w2["b_-_Employer_identification_number__EIN_"],
    "c_-_Employer_s_name__address__and_ZIP_code":
      w2["c_-_Employer_s_name__address__and_ZIP_code"],
  ...
  if (w2["XMLNAME_18_-_Local_wages__tips__etc_1"]) {
    const originalLocal1 = parseFloat(
      w2["XMLNAME_18_-_Local_wages__tips__etc_1"]
    );
    const correctedLocal1 = shouldChangeLocalWages
      ? originalLocal1 *
        faker.number.float({ min: 0.9, max: 1.1, fractionDigits: 2 })
      : originalLocal1;
    w2c["XMLNAME_18_-_Local_wages__tips__etc_1"] =
      roundCurrencyToString(originalLocal1);
    w2c["XMLNAME_18_-_Local_wages__tips__etc_1_W2C"] =
      roundCurrencyToString(correctedLocal1);
  ...
  if (w2["XMLNAME_18_-_Local_wages__tips__etc_2"]) {
    const originalLocal2 = parseFloat(
      w2["XMLNAME_18_-_Local_wages__tips__etc_2"]
    );
    const correctedLocal2 = shouldChangeLocalWages
      ? originalLocal2 *
        faker.number.float({ min: 0.9, max: 1.1, fractionDigits: 2 })
      : originalLocal2;
    w2c["XMLNAME_18_-_Local_wages__tips__etc_2"] =
      roundCurrencyToString(originalLocal2);
    w2c["XMLNAME_18_-_Local_wages__tips__etc_2_W2C"] =
      roundCurrencyToString(correctedLocal2);
    // Include locality name for matching
...
export function generateEmployeeData(numEmployees, calendarYear) {
  ...
  function generateUniqueEmployeeID() {
    ...
    do {
      ...
      if (attempts > maxAttempts) {
        throw new Error(
          "Failed to generate unique Employee_ID after maximum attempts"
        );
      }
  ...
  for (let i = 0; i < numEmployees; i++) {
    ...
    const numCompanies = faker.datatype.boolean({ probability: 0.1 })
      ...
      : 1;
    const employeeCompanies = faker.helpers.arrayElements(
      companies,
      numCompanies
    );
